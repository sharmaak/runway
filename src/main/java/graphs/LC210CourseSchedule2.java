...
/**
 * LeetCode problem 201 Course Schedule 2
...
public class LC210CourseSchedule2 {
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger( LC210CourseSchedule2.class );
    
    public int[] findOrder(int numCourses, int[][] prerequisites) {
        
        // 1. Prepare adjacency list for directed graph
        Map<Integer, List<Integer>> adj = toAdjacencyList(numCourses, prerequisites);
        
        // 2. Calculate in-degree for all vertices
        int[] inDegree = new int[numCourses];
        
        // The following iteration could have been over entrySet,
        // but iterated over keys to maintain the obviousness.
        ...
        // 3. Iterate over all nodes with in-degree = 0. Because there are
        //    source nodes (all its edges going away)
        ...
        return doTopologicalSort( queue, inDegree, adj );
    }
     ...
     */
    private int[] doTopologicalSort(Queue<Integer> queue, int[] inDegree,
        ...
        return visited != adj.keySet().size() ? new int[0] : topologicalSortResult;
    }
    
    
    private Map<Integer, List<Integer>> toAdjacencyList(int numCourses, int[][] prerequisites) {
